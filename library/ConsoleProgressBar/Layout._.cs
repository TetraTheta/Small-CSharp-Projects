...
namespace ConsoleProgressBar {
  ...
  public partial class Layout {
    ...
    /// </summary>
    public int GetInnerWidth(ProgressBar progressBar) => Math.Max(ProgressBarWidth - Margins.GetLength(progressBar), 0);
    ...
    internal List<Action> GetRenderActions(ProgressBar progressBar) {
      ...
      bool marqueeInProgress = progressBar.HasProgress && progressBar.MarqueePosition >= 0 && progressBar.MarqueePosition < progressLenght && Marquee.OverProgress.GetVisible(progressBar);
      bool marqueeInPending = progressBar.MarqueePosition >= progressLenght && Marquee.OverPending.GetVisible(progressBar);
      ...
      string innerText = "";
      if (Body.Text.GetVisible(progressBar)) innerText = (Body.Text.GetValue(progressBar) ?? "").AdaptToMaxWidth(innerWidth);

      string textProgressBeforeMarquee = string.IsNullOrEmpty(innerText) ? new string(Body.Progress.GetValue(progressBar), progressBeforeMarqueeLength) : innerText.Substring(0, progressBeforeMarqueeLength);

      char? charProgressMarquee = null;
      if (marqueeInProgress) charProgressMarquee = string.IsNullOrEmpty(innerText) ? Marquee.OverProgress.GetValue(progressBar) : innerText[progressBar.MarqueePosition];

      string textProgressAfterMarquee = string.IsNullOrEmpty(innerText) ? new string(Body.Progress.GetValue(progressBar), progressAfterMarqueeLength) : innerText.Substring(progressBar.MarqueePosition + 1, progressAfterMarqueeLength);

      string textPendingBeforeMarquee = string.IsNullOrEmpty(innerText) ? new string(Body.Pending.GetValue(progressBar), pendingBeforeMarqueeLength) : innerText.Substring(progressLenght, pendingBeforeMarqueeLength);

      char? charPendingMarquee = null;
      if (marqueeInPending) charPendingMarquee = string.IsNullOrEmpty(innerText) ? Marquee.OverPending.GetValue(progressBar) : innerText[progressBar.MarqueePosition];

      string textPendingAfterMarquee = string.IsNullOrEmpty(innerText) ? new string(Body.Pending.GetValue(progressBar), pendingAfterMarqueeLength) : innerText.Substring(progressBar.MarqueePosition + 1, pendingAfterMarqueeLength);
