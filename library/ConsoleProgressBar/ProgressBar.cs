...
namespace ConsoleProgressBar {
  ...
  public class ProgressBar : IDisposable {
    ...
    public void WriteLine(string value, ConsoleColor? foregroundColor = null, ConsoleColor? backgroundColor = null, bool truncateToOneLine = true) {
      ...
      if (FixedInBottom && _NumberLastLinesWritten > 0 && Console.CursorTop >= _ConsoleRow) Render();
    }
        ...
        if (FixedInBottom) {
          if (_ConsoleRow < Console.WindowHeight - _NumberLastLinesWritten) {
            ...
            if (_NumberLastLinesWritten > 0 && _ConsoleRow >= 0) {
              ...
              for (int i = _ConsoleRow; i < newConsoleRow; i++) Console.WriteLine(emptyLine);
            }
          ...
          if (_ConsoleRow - oldCursorTop <= scrollMargin / 2) {
            ...
            Console.SetCursorPosition(0, oldCursorTop);
            for (int i = oldCursorTop; i < _ConsoleRow + _NumberLastLinesWritten; i++) Console.WriteLine(emptyLine);
            _ConsoleRow = oldCursorTop + scrollMargin;
          }
        } else if (_ConsoleRow < 0) _ConsoleRow = oldCursorTop;
        Console.SetCursorPosition(0, _ConsoleRow);
    ...
    public void Unrender() {
      if (_ConsoleRow < 0 || _NumberLastLinesWritten <= 0) return;
      ...
      lock (ConsoleWriterLock) {
        ...
        if (FixedInBottom) initialRow = Math.Max(_ConsoleRow, oldCursorTop);

        //Position
        Console.SetCursorPosition(0, initialRow);

        //Remove lines
        for (int i = initialRow; i < _ConsoleRow + _NumberLastLinesWritten; i++) Console.WriteLine(emptyLine);
    ...
    private List<Action> GetRenderActionsForProgressBarAndText() {
      ...
      if (maxTextLenght >= 10) {
        ...
        if (text != null) list.AddRange(text.GetRenderActions(this, s => (" " + s).AdaptToMaxWidth(maxTextLenght)));
      }
      ...
      if (oldLinesWritten > _NumberLastLinesWritten) {
        for (int i = 0; i < oldLinesWritten - _NumberLastLinesWritten; i++) list.Add(() => Console.WriteLine(emptyLine));
      }
      return list;
    }
    ...
    private void UpdateMarqueePosition() {
      ...
      if (newProgressPosition < 0 || newProgressPosition >= Layout.GetInnerWidth(this)) MarqueeIncrement *= -1;
    ...
    public void Dispose() {
      ...
      //UpdateRemainingTime();
      Render();
      if (FixedInBottom && _NumberLastLinesWritten > 0 && _ConsoleRow >= 0) Console.CursorTop = _ConsoleRow + _NumberLastLinesWritten;

      if (ProgressStopwatch.IsRunning) ProgressStopwatch.Stop();
