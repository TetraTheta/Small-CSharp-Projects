...
namespace ConsoleProgressBar {
  public partial class Text {
    ...
    public class TextDescription {
      ...
      /// </summary>
      public Element<string> Indentation { get; } = new Element<string>();

      /// <summary>
      /// Ctor
      /// </summary>
      public TextDescription() {
        Processing.AddNew().SetValue(pb => pb.ElementName).SetVisible(pb => !string.IsNullOrEmpty(pb.ElementName)).SetForegroundColor(ConsoleColor.DarkYellow);

        Paused.AddNew().SetValue("[Paused]").SetForegroundColor(ConsoleColor.DarkCyan);

        Done.AddNew().SetValue(pb => $"{pb.Value} in {pb.TimeProcessing.ToStringWithAllHours()} ({pb.TimePerElement.ToStringWithAllHours()} each one)").SetForegroundColor(ConsoleColor.DarkGray);
